...
// Updated 10-Stage Lifecycle Pipeline
...
// Active borrowing tracker requests database (Mapped to new stage indices)
...
function renderTrackerList() {
  ...
  visibleRequests.forEach(req => {
    ...
    // Build responsive 10-step progress bar with horizontal overflow wrapping support
    const stepperHTML = lifecycleStages.map((stage, idx) => {
      ...
      return `
        <div class="step-item ${stepClass}" style="min-width: 65px; flex: 1;">
          <div class="step-circle">${idx + 1}</div>
          <span class="step-label" style="font-size: 10px; line-height: 1.1;">${stage}</span>
        </div>
      `;
    }).join('');
    ...
    card.innerHTML = `
      ...
      <div class="stepper-wrapper" style="overflow-x: auto; padding: 15px 0;">
        ${stepperHTML}
      </div>

      <div style="display: flex; justify-content: space-between; align-items: center; background: #fafafa; padding: 10px; border-radius: 6px; margin-top: 10px;">
        <div class="btn-group">
...
function resetItemStage(id) {
  activeUserRequests = activeUserRequests.map(req => {
    if (req.id === id) {
      req.stageIndex = 1; // Resets back to 'Requested'
